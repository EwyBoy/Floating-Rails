...
public class BlockMagmaLily extends BlockLilyPad implements IBlockRenderer, BlockLoader.IHasCustomItem {
    ...
    protected boolean canSustainBush(IBlockState state) {
        if (state.getBlock().equals(Blocks.FLOWING_LAVA)) {
            return false;
        } else {
    ...
    public boolean canPlaceBlockAt(World worldIn, BlockPos pos) {
        if (worldIn.getBlockState(pos.down()).equals(Blocks.FLOWING_LAVA)) {
            return false;
        } else {
    ...
    public boolean canBlockStay(World world, BlockPos pos, IBlockState state) {
        if (pos.getY() >= 0 && pos.getY() < 256) {
            ...
            if (iBlockState.getBlock().equals(Blocks.FLOWING_LAVA)) {
                return false;
            } else {
